...
ExecutorClient::ExecutorClient() {
  ...
  get_ordered_sub_goals_client_ = node_->create_client<plansys2_msgs::srv::GetOrderedSubGoals>(
      "executor/get_ordered_sub_goals");  // 创建获取有序子目标的服务客户端
  get_plan_client_ = node_->create_client<plansys2_msgs::srv::GetPlan>(
      "executor/get_plan");               // 创建获取计划的服务客户端
}
...
void ExecutorClient::createActionClient() {
  action_client_ =
      rclcpp_action::create_client<ExecutePlan>(node_, "execute_plan");  // 创建动作客户端

  if (!this->action_client_->wait_for_action_server(3s)) {               // 等待动作服务器
...
bool ExecutorClient::start_plan_execution(const plansys2_msgs::msg::Plan& plan) {
  if (!executing_plan_) {                       // 如果当前没有正在执行的计划
    createActionClient();                       // 创建动作客户端
    auto success = on_new_goal_received(plan);  // 处理新的目标

    if (success) {                              // 如果处理成功
      executing_plan_ = true;                   // 标记为正在执行计划
      return true;
    }
  } else {
    RCLCPP_INFO(node_->get_logger(), "Already executing a plan");  // 输出日志信息
  }
 ...
 * 4. 如果成功等到了服务，则创建一个 plansys2_msgs::srv::GetOrderedSubGoals::Request 类型的智能指针
 * request。
...
std::optional<plansys2_msgs::msg::Plan> ExecutorClient::getPlan() {
  ...
  if (rclcpp::spin_until_future_complete(node_, future_result, std::chrono::seconds(1)) !=
      rclcpp::FutureReturnCode::SUCCESS) {  // 等待获取计划的服务返回结果，如果超时则返回空
    return {};
  }
