...
LoggerNode::LoggerNode() : Node("plansys2_logger") {
...
void LoggerNode::knowledge_callback(plansys2_msgs::msg::Knowledge::SharedPtr msg) {
  std::string goal_set = (msg->goal == "") ? "goal not set" : "goal set";
  RCLCPP_INFO_STREAM(
      get_logger(), "[Knowledge] " << msg->instances.size() << " instances -- "
...
void LoggerNode::action_execution_info_callback(
    plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg) {
  ...
  output << "Completion: " << msg->completion << " -- Status message: " << msg->status << std::endl;
...
void LoggerNode::action_execution_callback(plansys2_msgs::msg::ActionExecution::SharedPtr msg) {
  std::ostringstream output;
...
void LoggerNode::action_performer_status_callback(
...
void LoggerNode::executing_plan_callback(plansys2_msgs::msg::Plan::SharedPtr msg) {
  if (msg->items.empty()) {
    RCLCPP_WARN_STREAM(get_logger(), "[Executing Plan] No plan for execution");
  } else {
    std::ostringstream output;
    output << "[Executing Plan] Executing Plan with " << msg->items.size() << " actions"
           << std::endl;
